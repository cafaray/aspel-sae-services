...
const SQL_FIND_ALL = `SELECT ${LIMIT} trim(p.CVE_PROV) CVE_PROV, s.NOMBRE, s.RFC, p.NUM_CARGO, 
    trim(c.DESCR), trim(p.REFER) REFER, trim(p.DOCTO) DOCTO, 
    ...
    ROUND(f.IMPUESTO1, 2) IMPUESTO1, ROUND(f.IMPUESTO2, 2) IMPUESTO2, ROUND(f.IMPUESTO3, 2) IMPUESTO3, ROUND(f.IMPUESTO4, 2) IMPUESTO4, 
    ROUND(f.IMPUESTO1 + f.IMPUESTO2 + f.IMPUESTO3, 2) AS IMPUESTOS, 
    ROUND(f.IMPUESTO1 + f.IMPUESTO2 + f.IMPUESTO3 - f.IMPUESTO4 + p.IMPORTE, 2) AS IMPORTE, 
...
const SQL_FINDBY_ID = `SELECT trim(p.CVE_PROV) CVE_PROV, s.NOMBRE, s.RFC, p.NUM_CARGO, 
...
async function getById(docto, cb) {
    try {
        const dbInstance = await getDBInstance()
        dbInstance.query(SQL_FINDBY_ID, [docto], (err, data) => {
            if(err){
                console.log(`${message}${err}`)
                setImmediate(() => cb(err))
            }
...
module.exports={
    getAll, getUnaccountedByDate, getUnaccountedByDateProvider, setUpgradeCOI, getUnaccountedByProvider, getById
}